...
with open("server_list.txt", "r") as servers:
        for current_server in servers.readlines():
            ...
            try:
                proc=subprocess.Popen("sshpass -p '2' ssh  -o 'StrictHostKeyChecking no' root@"+ current_server + " 'date +%W%m%Y'", shell=True, universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
